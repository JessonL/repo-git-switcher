...
class MonoSwitchCore {
  ...
  async safeSwitchBranch(repo, branch, options) {
    ...
    if (!branchExists) {
      if (create) {
        ...
        // 分支不存在，让用户选择其他分支
        const selectedBranch = await this.promptBranchSelection(repoName, repoPath, branch);
        if (selectedBranch) {
          // 使用用户选择的分支继续切换
   ...
   * 提示用户选择分支
   */
  async promptBranchSelection(repoName, repoPath, targetBranch) {
    console.log(chalk.yellow(`[${repoName}] 分支 ${targetBranch} 不存在，请选择其他分支:`));
