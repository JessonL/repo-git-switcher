...
class MonoSwitchCLI {
  ...
  setupProgram() {
    program
      .name('msg')
      .description(chalk.blue('@lee/mono-switchForGit - 安全的多仓库Git分支切换工具'))
...
${chalk.yellow('示例:')}
  ${chalk.cyan('msg switch develop')}                    # 切换到develop分支
  ${chalk.cyan('msg sw feature/new-feature -c')}         # 切换到新分支，不存在则创建
  ${chalk.cyan('msg switch main -p')}                    # 切换到main分支并拉取代码
  ${chalk.cyan('msg switch hotfix -f')}                  # 强制切换到hotfix分支
  ${chalk.cyan('msg list')}                              # 列出所有仓库状态
  ${chalk.cyan('msg ls -m "service-*"')}                 # 列出匹配service-*的项目
  ${chalk.cyan('msg status')}                            # 显示状态概览
    `);
  }
