...
class MonoSwitchCLI {
  ...
  setupProgram() {
    program.name('git-up').description(chalk.blue('@lee_0527/repo-git-switcher - 安全的多仓库Git分支切换工具')).version('1.0.0', '-v, --version', '显示版本号');
    ...
    program
      ...
      .option('-d, --directory <dir>', '指定包含Git仓库的目录', './packages')
      .option('-m, --match <pattern>', '只处理匹配指定模式的项目目录')
      .action(this.handleCleanLocal.bind(this));
...
${chalk.yellow('示例:')}
  ...
  ${chalk.cyan('git-up clean-local')}                       # 重置本地工作区的修改
    `
    );
  }
  ...
  async handleCleanLocal(options) {
    try {
      const { cleanLocal } = require('./core');
      await cleanLocal(options.directory, options.match);
    } catch (error) {
      console.error(chalk.red('错误:'), error.message);
      process.exit(1);
    }
